...
class Db
{
    ...
    public function factory($db_config='')
    {
        ...
        $dbClass = "core\\Model\\Driver\\".$dbClass;
        // 检查驱动类
    ...
    protected function multiConnect($master=false) {
        ...
        //20141105 zzjin
        //增加了逻辑,如果是读写分离,切有一次写入数据的话,之后的读都走写的数据库直接读,避免数据库之间同步不及时
        ...
        if(C('DB_RW_SEPARATE')){
            ...
            if($master){
                ...
                // 读操作连接从服务器
                if (isset($_config['writed']) && $_config['writed'] == true){
                    //还是走主数据库
                    $r = 0;
                }else{
    ...
    protected function debug() {
        ...
        if ( $this->debug ) {
            G('queryEndTime');
            $m_runtime = G('queryStartTime','queryEndTime',6);
            //Log::record($this->queryStr." [ RunTime:".$m_runtime."s ]",Log::SQL);
            trace($this->queryStr.' [ RunTime:'.$m_runtime.'s ]','','SQL');
            global $model_time;
            if ($m_runtime >= $model_time['model_runtime']){
                $model_time['model_runtime'] = $m_runtime;
            }
        }
    }
